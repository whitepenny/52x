...
function ftx_get_the_content( $post_id = false ) {
  if ( ! empty( $post_id ) ) {
...
function ftx_external_link( $link ) {
  global $ftx_domain;

  return ( strpos( $link, $ftx_domain ) === false );
}


// Draw Link Target

function ftx_link_target( $link ) {
  return ftx_external_link( $link ) ? ' target="_blank"' : '';
}


// Get Template Part w/ Variables

function ftx_template_part( $template, $args ) {
  // $path = locate_template( $template . '.php', true, false );
  $path = FTX_THEME_DIR . '/' . $template . '.php';
...
function ftx_main_navigation( $depth = 2 ) {
  $args = array(
...
// // Sub Nav
//
// function ftx_sub_navigation( $depth = 2 ) {
//   $args = array(
//     'theme_location' => 'main-navigation',
//     'depth'          => $depth
//   );
...
function ftx_menu_item_classes( $classes, $item, $args ) {
  global $ftx_domain;
  global $ftx_page_url;
  ...
  if ( ! empty( $cpt ) ) {
    ...
    $item_path = str_ireplace( $ftx_domain, '', $item->url );
    $current_path = str_ireplace( $ftx_domain, '', $ftx_page_url );
    ...
    if ( strpos( $ftx_page_url, $item_path ) !== false ) {
...
add_filter( 'nav_menu_css_class', 'ftx_menu_item_classes', 10, 3 );


// Footer Nav

function ftx_footer_navigation( $depth = 1 ) {
  $args = array(
...
function ftx_subfooter_navigation( $depth = 1 ) {
  $args = array(
...
function ftx_format_content( $text = '', $echo = true ) {
  $find    = array( '[br]', '{', '}' );
...
function ftx_json_options( $options = array(), $echo = true ) {
...
function ftx_get_oembed_url( $html ) {
  if ( empty( $html ) ) {
    return;
  }
...
function ftx_get_oembed_data( $url ) {
...
function ftx_trim_length( $string = '', $length ) {
  $ns = '';
  $opentags = array();
  $string = trim( $string );
